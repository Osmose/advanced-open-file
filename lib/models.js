 ...
 */
export class Path {
    static cached_stat = Symbol('cached_stat');
    ...
    async stat() {
        if (this[Path.cached_stat] === undefined) {
            try {
                this[Path.cached_stat] = await fsp.stat(absolutify(this.full));
            } catch (err) {
                if (err.code === 'ENOENT') {
                    this[Path.cached_stat] = null;
                } else {
                    throw err;
                }
            }
    ...
    async matchingPaths(caseSensitive=null) {
        let absoluteDir = absolutify(this.directory);
        let filenames = null;

        try {
            filenames = await fsp.readdir(absoluteDir);
        } catch (err) {
