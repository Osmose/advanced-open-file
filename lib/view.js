...
export default class AdvancedOpenFileView {
    constructor() {
        ...
        this.pathEditor.onDidChange(() => {
            if (!this._updatingPath) {
                this.setPath(new Path(this.pathEditor.getText()));
            }
        });
    ...
    async createPathListItem(path) {
        let icon = path.isDirectory() ? 'icon-file-directory' : 'icon-file-text';
        let status = await this.currentDirectoryPath.gitStatus(path.full);
        return `
            <li class="list-item ${path.isDirectory() ? 'directory' : ''} ${status}"
     ...
     */
    async refreshPathListItem(path) {
        let listItem = this.content.querySelector(`.list-item[data-file-name="${path.full}"]`);
        if (listItem) {
            let newListItem = dom(await this.createPathListItem(path));
    ...
    setPath(path) {
        ...
        // Only update if the directory has changed to take advantage of
        // the caching on the model.
        let directoryPath = path.asDirectory();
        if (this.currentDirectoryPath != directoryPath) {
        ...
        this.setPathList(path.matchingPaths(), {
            hideParent: path.fragment !== '' || path.isRoot(),
            sort: !(config.get('fuzzyMatch') && path.fragment !== ''),
        });
    ...
    async _appendToPathList(paths) {
        for (path of paths) {
            if (path.exists()) {
                let listItem = dom(await this.createPathListItem(path));
