...
export default class AdvancedOpenFileView {
    ...
    get selectedPath() {
        let {path, selectedIndex, matchingPaths} = this.content.state;
        if (!matchingPaths || selectedIndex === null) {
            return null;
        } else {
            return matchingPaths[selectedIndex];
        }
    }

    get firstPath() {
        let {matchingPaths} = this.content.state;
    ...
    setPath(path) {
        this.content.setState({
            path: path,
            selectedIndex: null,
            updateMatchingPaths: true
        });
    }

    moveCursorDown() {
        this.content.setState((state, props) => {
            let {matchingPaths, selectedIndex} = state;
            if (!matchingPaths) {
                return {};
            }

            if (selectedIndex === null || selectedIndex === matchingPaths.length - 1) {
                selectedIndex = 0;
            } else {
                selectedIndex++;
            }
    ...
    moveCursorUp() {
        this.content.setState((state, props) => {
            ...
            if (selectedIndex === null || selectedIndex === 0) {
                selectedIndex = matchingPaths.length - 1;
            } else {
                selectedIndex--;
            }
...
export class AdvancedOpenFileContent extends Component {
    ...
    async fetchMatchingPaths(path) {
        let matchingPaths = await fileService.getMatchingPaths(path);
    ...
    componentDidUpdate() {
        if (this.state.updateMatchingPaths) {
            setTimeout(() => {
                if (this.state.matchingPaths !== null) {
                    this.setState(state => {
                        if (state.updateMatchingPaths) {
                            return {
                                updateMatchingPaths: false,
                                matchingPaths: null
                            };
                        } else {
                            return {};
            ...
            this.fetchMatchingPaths(this.state.path).then(matchingPaths => {
                this.setState({matchingPaths, updateMatchingPaths: false});
            });
        }
    ...
    render() {
        let {path, selectedIndex, matchingPaths} = this.state;
        ...
        if (matchingPaths !== null) {
            ...
            pathList = <LoadingPathListIndicator />;
        }
    ...
    handlePathInputChange(newText) {
        ...
        this.setState({path: newPath, selectedIndex: null, updateMatchingPaths: true});
...
export class PathListItem extends Component {
    render() {
        ...
        return (
            <div className={liClassName} onClick={::this.handleClick} data-file-name={path.full}>
...
export function LoadingPathListIndicator() {
    return (
        <div className="loading-indicator settings-view">
            <div className="alert alert-info icon icon-hourglass">
                Loading path list...
            </div>
        </div>
    )
