...
export default class AdvancedOpenFileView {
    ...
    parentDirectoryListItem(path) {
        return `
            <li class="list-item parent-directory"
                data-file-name="${path.parent().full}">
                <span class="icon icon-file-directory">..</span>
            </li>
        `;
    }

    async pathListItem(path) {
        let isDirectory = await path.isDirectory();
        let icon = isDirectory ? 'icon-file-directory' : 'icon-file-text';
        return `
            <li class="list-item ${isDirectory ? 'directory' : ''}"
                data-file-name="${path.full}">
                <span class="filename icon ${icon}"
                      data-name="${path.fragment}">
                    ${path.fragment}
                </span>
                ${isDirectory && !path.isProjectDirectory()
    ...
    async setPath(path) {
        ...
        let newPathList = this.pathListElement();
        let paths = await path.matchingPaths();
        this.cursorIndex = null;
        ...
        if (paths.length > 0) {
            ...
            if (path.fragment === '') {
                newPathList.appendChild(this.parentDirectoryListItem(path));
            }

            for (path of directoryPaths) {
                let html = await this.pathListItem(path);
                newPathList.appendChild(dom(html));
            }
        ...
        this.content.replaceChild(newPathList, this.pathList);
