...
export class PathListItem extends Component {
    render() {
        ...
        let pathIsDirectory = fileService.isDirectory(path);
        let icon = pathIsDirectory ? 'icon-file-directory' : 'icon-file-text';

        let addProjectFolderButton = '';
        if (pathIsDirectory && !path.isProjectDirectory() && !path.displayAsParent) {
