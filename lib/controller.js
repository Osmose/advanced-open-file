...
export class AdvancedOpenFileController {
    ...
    async pathChange(newPath)  {
        ...
        if (config.get('helmDirSwitch')) {
            if (newPath.hasShortcut('')) {  // Empty shortcut == '//'
                ...
                newPath = await fileService.pathFor(osenv.home() + stdPath.sep);
                replace = true;
            } else if (newPath.hasShortcut(':')) {
                let projectPath = getProjectPath();
                if (projectPath) {
                    newPath = await fileService.pathFor(projectPath + newPath.sep);
                    replace = true;
                }
            }
        }
    ...
    async openPath(path) {
        if (path.exists()) {
            ...
            try {
                await fileService.createDirectories(path);
     ...
     */
    async autocomplete() {
        let matchingPaths = await fileService.getMatchingPaths(this.currentPath);
