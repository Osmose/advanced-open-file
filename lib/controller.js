...
export class AdvancedOpenFileController {
    constructor() {
        ...
        this.disposables.add(atom.commands.add('.advanced-open-file', {
            ...
            'advanced-open-file:move-cursor-down': ::this.view.moveCursorDown,
            'advanced-open-file:move-cursor-up': ::this.view.moveCursorUp,
            'advanced-open-file:move-cursor-bottom': ::this.view.moveCursorBottom,
            'advanced-open-file:move-cursor-top': ::this.view.moveCursorTop,
        ...
        this.view.onDidClickPath(::this.clickPath);
    ...
    selectPath(newPath, split=false) {
        if (fileService.isDirectory(newPath)) {
            if (split !== false) {
                atom.beep();
            } else {
    ...
    addProjectFolder(folderPath) {
        if (fileService.isDirectory(folderPath) && !folderPath.isProjectDirectory()) {
     ...
     */
    autocomplete() {
        let matchingPaths = fileService.getMatchingPaths(this.currentPath);
